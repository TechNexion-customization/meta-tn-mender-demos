SUMMARY = "Wrap a directory into a .mender artifact for mender-directory update module plugin"
LICENSE = "MIT"
...
MENDER_ARTIFACT_FILE_TREE ?= ""
MENDER_ARTIFACT_FILE_TREE_DEST_DIR ?= ""

# --------------------------- CONCEPT EXPLANATION -----------------------------
#
# Use the directory-artifact-gen from mender-directory-native to create the .mender artifact
#
# NOTE: directory-artifact-gen calls mender-artifact x86_64 binary (from mender-artifact-native)
...
# ./directory-artifact-gen -n ${ARTIFACT_NAME} -t ${DEVICE_TYPE} -d ${DEST_DIR} -o ${OUTPUT_PATH} ${FILE_TREE} -- ${MENDER_ARTIFACT_EXTRA_ARGS}
#

inherit deploy

DEPENDS += "mender-directory-native mender-artifact-native"
...
do_deploy[depends] = "mender-directory-native:do_populate_sysroot mender-artifact-native:do_populate_sysroot"

do_deploy () {
	bbnote "Generate mender-directory-artifact: ${OUTPUT_PATH}"

	# generate the mender directory artifact
	directory-artifact-gen -n ${ARTIFACT_NAME} -t ${MACHINE} -d ${MENDER_ARTIFACT_FILE_TREE_DEST_DIR} -o ${OUTPUT_PATH} ${MENDER_ARTIFACT_FILE_TREE} -- ${MENDER_ARTIFACT_EXTRA_ARGS}
